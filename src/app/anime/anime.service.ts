...
import { AnimeElement } from 'src/app/query-data/anime-data';
...
export class AnimeService {
  ...
  async GetAnime(_id: number): Promise<AnimeElement> {
    var query = {
      query: this._query,
      variables: {
        id: _id,
    ...
    const response = this.http.post<AnimeElement>(this.url, query);
